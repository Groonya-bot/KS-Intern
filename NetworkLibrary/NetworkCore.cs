            ...
            while (cl.Connected)
            {
                ...
                try
                {
                    ...
                    if (result == "!time")
                    {
                        ...
                        MessageEvent?.Invoke(datetime, client);
                        if (result == "!ntr")
                        {
                            string epic = " Полны чудес сказанья давно минувших дней/nПро громкие деянья былых богатырей./nИ распри их кровавые услышите вы вскоре.";
                            Debug.WriteLine("Received from client: " + result);
                            MessageEvent?.Invoke( epic, client);
                        }
                    }
                    else
                    {
